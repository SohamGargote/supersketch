const demention = 15;

export const Pencil = () => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 512 512"
    height={demention}
    fill="currentcolor"
  >
    <path d="M410.3 231l11.3-11.3-33.9-33.9-62.1-62.1-33.9-33.9-11.3 11.3-22.6 22.6L58.6 322.9c-10.4 10.4-18 23.3-22.2 37.4L1 480.7c-2.5 8.4-.2 17.5 6.1 23.7s15.3 8.5 23.7 6.1l120.3-35.4c14.1-4.2 27-11.8 37.4-22.2l199.2-199.2 22.6-22.7zM160 399.4l-9.1 22.7c-4 3.1-8.5 5.4-13.3 6.9l-78.2 23 23-78.1c1.4-4.9 3.8-9.4 6.9-13.3l22.7-9.1v32c0 8.8 7.2 16 16 16h32zM362.7 18.7l-14.4 14.5-22.6 22.6-11.4 11.3 33.9 33.9 62.1 62.1 33.9 33.9 11.3-11.3 22.6-22.6 14.5-14.5c25-25 25-65.5 0-90.5l-39.3-39.4c-25-25-65.5-25-90.5 0zm-47.4 168l-144 144c-6.2 6.2-16.4 6.2-22.6 0s-6.2-16.4 0-22.6l144-144c6.2-6.2 16.4-6.2 22.6 0s6.2 16.4 0 22.6z"></path>
  </svg>
);

export const Eraser = () => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 576 512"
    height={demention}
    fill="currentcolor"
  >
    <path d="M290.7 57.4L57.4 290.7c-25 25-25 65.5 0 90.5l80 80c12 12 28.3 18.7 45.3 18.7H512c17.7 0 32-14.3 32-32s-14.3-32-32-32H387.9l130.7-130.6c25-25 25-65.5 0-90.5L381.3 57.4c-25-25-65.5-25-90.5 0zm6.7 358.6H182.6l-80-80 124.7-124.7 137.4 137.4-67.3 67.3z"></path>
  </svg>
);

export const Circle = () => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 512 512"
    height={demention}
    fill="currentcolor"
  >
    <path d="M464 256a208 208 0 10-416 0 208 208 0 10416 0zM0 256a256 256 0 11512 0 256 256 0 11-512 0z"></path>
  </svg>
);

export const Rectangle = () => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 448 512"
    height={demention}
    fill="currentcolor"
  >
    <path d="M384 80c8.8 0 16 7.2 16 16v320c0 8.8-7.2 16-16 16H64c-8.8 0-16-7.2-16-16V96c0-8.8 7.2-16 16-16h320zM64 32C28.7 32 0 60.7 0 96v320c0 35.3 28.7 64 64 64h320c35.3 0 64-28.7 64-64V96c0-35.3-28.7-64-64-64H64z"></path>
  </svg>
);

export const Line = () => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 290.658 290.658"
    fill="currentcolor"
    stroke="currentcolor"
    strokeWidth="21"
    height={demention}
  >
    <path d="M0 139.474H290.658V151.185H0z"></path>
  </svg>
);

export const Arrow = () => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width={20}
    fill="none"
    stroke="currentcolor"
    strokeLinecap="round"
    strokeLinejoin="round"
    strokeMiterlimit="10"
    strokeWidth="2"
    version="1.1"
    viewBox="0 0 32 32"
    xmlSpace="preserve"
  >
    <path d="M21 10l6 6-6 6m6-6H5" className="st0"></path>
  </svg>
);

export const Selection = () => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 320 512"
    fill="currentcolor"
    height={demention}
  >
    <path d="M0 55.2V426c0 12.2 9.9 22 22 22 6.3 0 12.4-2.7 16.6-7.5l82.6-94.5 58.1 116.3c7.9 15.8 27.1 22.2 42.9 14.3s22.2-27.1 14.3-42.9L179.8 320h118.1c12.2 0 22.1-9.9 22.1-22.1 0-6.3-2.7-12.3-7.4-16.5L38.6 37.9c-4.3-3.8-9.7-5.9-15.4-5.9C10.4 32 0 42.4 0 55.2z"></path>
  </svg>
);

export const Diamond = () => (
  <svg
    fill="none"
    height={demention}
    stroke="currentColor"
    strokeWidth="2"
    viewBox="0 0 24 24"
  >
    <path d="M10.5 20.4l-6.9-6.9c-.781-.781-.781-2.219 0-3l6.9-6.9c.781-.781 2.219-.781 3 0l6.9 6.9c.781.781.781 2.219 0 3l-6.9 6.9c-.781.781-2.219.781-3 0z"></path>
  </svg>
);

export const Hand = () => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    height={demention}
    fill="currentColor"
    stroke="currentColor"
    strokeWidth="5"
    viewBox="-14.55 -14.55 514.1 514.1"
  >
    <path d="M382.5 69.429c-7.441 0-14.5 1.646-20.852 4.573-4.309-23.218-24.7-40.859-49.148-40.859a49.685 49.685 0 00-21.467 4.852C285.641 16.205 265.932 0 242.5 0c-23.432 0-43.141 16.206-48.533 37.995a49.696 49.696 0 00-21.467-4.852c-27.57 0-50 22.43-50 50v122.222a49.702 49.702 0 00-20-4.187c-27.57 0-50 22.43-50 50V354c0 72.233 58.766 131 131 131h118c72.233 0 131-58.767 131-131V119.429c0-27.571-22.43-50-50-50zM402.5 354c0 55.691-45.309 101-101 101h-118c-55.691 0-101-45.309-101-101V251.178c0-11.028 8.972-20 20-20s20 8.972 20 20v80h30V83.143c0-11.028 8.972-20 20-20s20 8.972 20 20v158.035h30V50c0-11.028 8.972-20 20-20s20 8.972 20 20v191.178h30V83.143c0-11.028 8.972-20 20-20s20 8.972 20 20v158.035h30v-121.75c0-11.028 8.972-20 20-20s20 8.972 20 20V354z"></path>
  </svg>
);

export const Lock = () => (
  <svg
    height={demention}
    fill="none"
    stroke="currentColor"
    strokeLinecap="round"
    strokeLinejoin="round"
    strokeWidth={2}
    viewBox="0 0 24 24"
  >
    <rect width={18} height={11} x={3} y={11} rx={2} ry={2} />
    <path d="M7 11V7a5 5 0 0 1 10 0v4" />
  </svg>
);

export const SolidLine = () => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    height={demention}
    fill="currentColor"
    stroke="currentColor"
    strokeWidth="29.066"
    version="1.1"
    viewBox="-29.07 -29.07 348.8 348.8"
    xmlSpace="preserve"
  >
    <path fill="currentColor" d="M0 139.474H290.658V151.185H0z"></path>
  </svg>
);

export const DashedLine = () => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 17 17"
    height={demention}
  >
    <path
      fill="currentColor"
      d="M0 8h4v1H0V8zm6.5 1h4V8h-4v1zM13 8v1h4V8h-4z"
    ></path>
  </svg>
);

export const DottedLine = () => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    height={demention}
    fill="currentColor"
    viewBox="0 0 24 24"
  >
    <path d="M3 13H1v-2h2zm4-2H5v2h2zm12 0h-2v2h2zm4 0h-2v2h2zm-12 0H9v2h2zm4 0h-2v2h2z"></path>
  </svg>
);

export const Duplicate = () => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="30 30 205 205"
    height={demention}
    fill="currentColor"
  >
    <path d="M47.81 91.725c0-8.328 6.539-15.315 15.568-15.33 9.03-.016 14.863.015 14.863.015s-.388-8.9-.388-15.978c0-7.08 6.227-14.165 15.262-14.165s92.802-.26 101.297.37c8.495.63 15.256 5.973 15.256 14.567 0 8.594-.054 93.807-.054 101.7 0 7.892-7.08 15.063-15.858 15.162-8.778.1-14.727-.1-14.727-.1s.323 9.97.323 16.094c0 6.123-7.12 15.016-15.474 15.016s-93.117.542-101.205.542c-8.088 0-15.552-7.116-15.207-15.987.345-8.871.345-93.58.345-101.906zm46.06-28.487l-.068 98.164c0 1.096.894 1.99 1.999 1.984l95.555-.51a2.007 2.007 0 001.998-2.01l-.064-97.283a2.01 2.01 0 00-2.01-2.007l-95.4-.326a1.99 1.99 0 00-2.01 1.988zM63.268 95.795l.916 96.246a2.007 2.007 0 002.02 1.982l94.125-.715a3.976 3.976 0 003.953-4.026l-.137-11.137s-62.877.578-71.054.578-15.438-7.74-15.438-16.45c0-8.71.588-68.7.588-68.7.01-1.1-.874-1.99-1.976-1.975l-9.027.13a4.025 4.025 0 00-3.97 4.067z"></path>
  </svg>
);

export const Delete = () => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    height={demention}
    viewBox="1 1 22 22"
    stroke="currentColor"
    fill="none"
    strokeLinecap="round"
    strokeLinejoin="round"
    strokeWidth="2"
  >
    <path d="M10 12v5M14 12v5M4 7h16M6 10v8a3 3 0 003 3h6a3 3 0 003-3v-8M9 5a2 2 0 012-2h2a2 2 0 012 2v2H9V5z"></path>
  </svg>
);

export const Backward = () => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    height={demention}
    fill="currentColor"
    enableBackground="new 0 0 512 512"
    viewBox="0 0 512 512"
  >
    <path d="M371.2 115.2H224c-60.09 0-108.8 48.71-108.8 108.8v147.2h-32C37.25 371.2 0 333.95 0 288V83.2C0 37.25 37.25 0 83.2 0H288c45.95 0 83.2 37.25 83.2 83.2v32zM512 428.8c0 45.95-37.25 83.2-83.2 83.2H224c-45.95 0-83.2-37.25-83.2-83.2V224c0-45.95 37.25-83.2 83.2-83.2h204.8c45.95 0 83.2 37.25 83.2 83.2v204.8zm-83.2 44.8c24.74 0 44.8-20.06 44.8-44.8V224c0-24.74-20.06-44.8-44.8-44.8H224c-24.74 0-44.8 20.06-44.8 44.8v204.8c0 24.74 20.06 44.8 44.8 44.8h204.8z"></path>
  </svg>
);

export const Forward = () => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    height={demention}
    fill="currentColor"
    enableBackground="new 0 0 512 512"
    viewBox="0 0 512 512"
  >
    <path d="M288 371.2c45.95 0 83.2-37.25 83.2-83.2V83.2C371.2 37.25 333.95 0 288 0H83.2C37.25 0 0 37.25 0 83.2V288c0 45.95 37.25 83.2 83.2 83.2H288zm-147.2 25.6h38.4v32c0 24.74 20.06 44.8 44.8 44.8h204.8c24.74 0 44.8-20.06 44.8-44.8V224c0-24.74-20.06-44.8-44.8-44.8h-32v-38.4h32c45.95 0 83.2 37.25 83.2 83.2v204.8c0 45.95-37.25 83.2-83.2 83.2H224c-45.95 0-83.2-37.25-83.2-83.2v-32z"></path>
  </svg>
);

export const ToBack = () => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    enableBackground="new 0 0 512 512"
    viewBox="0 0 512 512"
    height={demention}
    fill="currentColor"
  >
    <path d="M116.36 232.73h29.09c48.2 0 87.27-39.07 87.27-87.27v-29.09H320c41.77 0 75.64 33.86 75.64 75.64v87.27h-29.09c-48.2 0-87.27 39.07-87.27 87.27v29.09H192c-41.77 0-75.64-33.86-75.64-75.64v-87.27zM0 64C0 28.65 28.65 0 64 0h81.45c35.35 0 64 28.65 64 64v81.45c0 35.35-28.65 64-64 64H64c-35.35 0-64-28.65-64-64V64zm64-29.09c-16.07 0-29.09 13.02-29.09 29.09v81.45c0 16.07 13.02 29.09 29.09 29.09h81.45c16.07 0 29.09-13.02 29.09-29.09V64c0-16.07-13.02-29.09-29.09-29.09H64zm238.55 331.64c0-35.35 28.65-64 64-64H448c35.35 0 64 28.65 64 64V448c0 35.35-28.65 64-64 64h-81.45c-35.35 0-64-28.65-64-64v-81.45zm64-29.1c-16.07 0-29.09 13.02-29.09 29.09V448c0 16.07 13.02 29.09 29.09 29.09H448c16.07 0 29.09-13.02 29.09-29.09v-81.45c0-16.07-13.02-29.09-29.09-29.09h-81.45z"></path>
  </svg>
);

export const ToFront = () => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    height={demention}
    fill="currentColor"
    enableBackground="new 0 0 512 512"
    viewBox="0 0 512 512"
  >
    <path d="M0 64C0 28.65 28.65 0 64 0h81.45c35.35 0 64 28.65 64 64v29.09H192c-5.96 0-11.79.53-17.45 1.54V64c0-16.07-13.02-29.09-29.09-29.09H64c-16.07 0-29.09 13.02-29.09 29.09v81.45c0 16.07 13.02 29.09 29.09 29.09h30.63a99.43 99.43 0 00-1.54 17.45v17.45H64c-35.35 0-64-28.65-64-64V64zm302.55 354.91V448c0 35.35 28.65 64 64 64H448c35.35 0 64-28.65 64-64v-81.45c0-35.35-28.65-64-64-64h-29.09V320c0 5.96-.53 11.79-1.54 17.45H448c16.07 0 29.09 13.02 29.09 29.09V448c0 16.07-13.02 29.09-29.09 29.09h-81.45c-16.07 0-29.09-13.02-29.09-29.09v-30.63a99.43 99.43 0 01-17.45 1.54h-17.46zM116.36 192c0-41.77 33.86-75.64 75.64-75.64h128c41.77 0 75.64 33.86 75.64 75.64v128c0 41.77-33.86 75.64-75.64 75.64H192c-41.77 0-75.64-33.86-75.64-75.64V192z"></path>
  </svg>
);

export const Undo = () => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    height={demention}
    fill="currentColor"
    viewBox="0 0 24 24"
  >
    <path d="M12.5 8c-2.65 0-5.05.99-6.9 2.6L3.71 8.71C3.08 8.08 2 8.52 2 9.41V15c0 .55.45 1 1 1h5.59c.89 0 1.34-1.08.71-1.71l-1.91-1.91c1.39-1.16 3.16-1.88 5.12-1.88 3.16 0 5.89 1.84 7.19 4.5.27.56.91.84 1.5.64.71-.23 1.07-1.04.75-1.72C20.23 10.42 16.65 8 12.5 8z"></path>
  </svg>
);

export const Redo = () => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    height={demention}
    fill="currentColor"
    viewBox="0 0 24 24"
  >
    <path d="M18.4 10.6C16.55 8.99 14.15 8 11.5 8c-4.16 0-7.74 2.42-9.44 5.93-.32.67.04 1.47.75 1.71.59.2 1.23-.08 1.5-.64 1.3-2.66 4.03-4.5 7.19-4.5 1.95 0 3.73.72 5.12 1.88l-1.91 1.91c-.63.63-.19 1.71.7 1.71H21c.55 0 1-.45 1-1V9.41c0-.89-1.08-1.34-1.71-.71l-1.89 1.9z"></path>
  </svg>
);

export const MenuIcon = () => (
  <svg
    height="21"
    width="21"
    fill="none"
    stroke="currentColor"
    strokeLinecap="round"
    strokeLinejoin="round"
    strokeWidth={2}
    viewBox="0 0 24 24"
  >
    <path d="M3 12h18" />
    <path d="M3 6h18" />
    <path d="M3 18h18" />
  </svg>
);

export const Xmark = () => (
  <svg
    width={21}
    height={21}
    fill="none"
    stroke="currentColor"
    strokeLinecap="round"
    strokeLinejoin="round"
    strokeWidth={2}
    viewBox="0 0 24 24"
  >
    <path d="M18 6 6 18" />
    <path d="m6 6 12 12" />
  </svg>
);

export const Folder = () => (
  <svg
    height={demention}
    fill="none"
    stroke="currentColor"
    strokeLinecap="round"
    strokeLinejoin="round"
    strokeWidth={2}
    viewBox="0 0 24 24"
  >
    <path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z" />
  </svg>
);

export const Download = () => (
  <svg
    height={demention}
    fill="none"
    stroke="currentColor"
    strokeLinecap="round"
    strokeLinejoin="round"
    strokeWidth={2}
    viewBox="0 0 24 24"
  >
    <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" />
    <path d="m7 10 5 5 5-5" />
    <path d="M12 15V3" />
  </svg>
);

export const Image = () => (
  <svg
    height={demention}
    fill="none"
    stroke="currentColor"
    strokeLinecap="round"
    strokeLinejoin="round"
    strokeWidth={2}
    viewBox="0 0 24 24"
  >
    <rect width={18} height={18} x={3} y={3} rx={2} ry={2} />
    <path d="M8.5 7a1.5 1.5 0 1 0 0 3 1.5 1.5 0 1 0 0-3z" />
    <path d="m21 15-5-5L5 21" />
  </svg>
);

export const Github = () => (
  <svg
    height={demention}
    fill="none"
    stroke="currentColor"
    strokeLinecap="round"
    strokeLinejoin="round"
    strokeWidth={2}
    viewBox="0 0 24 24"
  >
    <path d="M16.25 22.5v-3.865a3.361 3.361 0 0 0-.94-2.607c3.14-.35 6.44-1.538 6.44-6.99a5.43 5.43 0 0 0-1.5-3.746 5.058 5.058 0 0 0-.09-3.765s-1.18-.35-3.91 1.478a13.397 13.397 0 0 0-7 0C6.52 1.177 5.34 1.527 5.34 1.527a5.058 5.058 0 0 0-.09 3.765 5.43 5.43 0 0 0-1.5 3.775c0 5.413 3.3 6.602 6.44 6.991a3.366 3.366 0 0 0-.94 2.577V22.5" />
    <path d="M9.25 19.503c-5 1.498-5-2.496-7-2.996" />
  </svg>
);

export const Type = () => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 384 512"
    height={demention}
    fill="currentcolor"
  >
    <path d="M0 32C0 14.3 14.3 0 32 0H352c17.7 0 32 14.3 32 32s-14.3 32-32 32H32C14.3 64 0 49.7 0 32zm64 96H320V448c0 17.7-14.3 32-32 32s-32-14.3-32-32V352H128V448c0 17.7-14.3 32-32 32s-32-14.3-32-32V128z"></path>
  </svg>
);

export const Brush = ({ demention = 24 }) => {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 512 512"
      height={demention}
      fill="currentcolor"
    >
      <path d="M126.8 219.3c4.5-4.5 7.1-10.8 7.1-17.3s-2.7-12.7-7.1-17.3l-26-26c-4.5-4.5-10.8-7.1-17.3-7.1l-33.8 33.8c-28.1 28.1-28.1 73.7 0 101.8l87.5 87.5c28.1 28.1 73.7 28.1 101.8 0l33.8-33.8c4.5-4.5 7.1-10.8 7.1-17.3s-2.7-12.7-7.1-17.3l-26-26c-4.5-4.5-10.8-7.1-17.3-7.1l-40.1 40.1-52.2-52.2 40.1-40.1zm-71-13.8l11.5 11.5c1.5 1.5 3.5 2.3 5.6 2.3s4.1-.8 5.6-2.3l26-26c1.5-1.5 2.3-3.5 2.3-5.6s-.8-4.1-2.3-5.6l-11.5-11.5-37.2 37.2zM187.8 291.8l-52.2-52.2 40.1-40.1 52.2 52.2-40.1 40.1zM264.9 219.3l-26-26c-4.5-4.5-10.8-7.1-17.3-7.1s-12.7 2.7-17.3 7.1l-26 26c-4.5 4.5-7.1 10.8-7.1 17.3s2.7 12.7 7.1 17.3l26 26c4.5 4.5 10.8 7.1 17.3 7.1s12.7-2.7 17.3-7.1l26-26c4.5-4.5 7.1-10.8 7.1-17.3s-2.7-12.7-7.1-17.3zm-33.9 33.9l-11.5-11.5c-1.5-1.5-3.5-2.3-5.6-2.3s-4.1.8-5.6 2.3l-26 26c-1.5 1.5-2.3 3.5-2.3 5.6s.8 4.1 2.3 5.6l11.5 11.5 37.2-37.2z" />
    </svg>
  );
};
